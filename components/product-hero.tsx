 ...
 * Also wires the gallery main-image click to trigger the buy-box add-to-cart.
 */
export function ProductHero({ formula, formulaKey }: ProductHeroProps) {
  ...
  const addToCartRef = useRef<(() => void) | null>(null)
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)] gap-8 lg:gap-12 items-start">
      <ProductGallery
        ...
        onMainImageClick={() => addToCartRef.current?.()}
      ...
      <BuyBox
        ...
        flavorId={flavorId}
        onFlavorChange={setFlavorId}
        onAddToCartReady={(fn) => { addToCartRef.current = fn }}
      />
    </div>
  )
}
